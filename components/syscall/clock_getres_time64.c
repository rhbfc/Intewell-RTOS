 ...
 * @param[out] ts 指向 timespec64 结构体的指针，用于返回时钟分辨率。
 ...
 */
DEFINE_SYSCALL (clock_getres_time64,
                 (clockid_t clk, struct timespec64 __user *ts))
{
    ...
    return copy_to_user (ts, &tp, sizeof (tp));
}
