...
#ifdef VIRTIO_CACHED_VRINGS
#define VRING_FLUSH(x)      cache_dcache_flush (&x, sizeof (x))
#define VRING_INVALIDATE(x) cache_dcache_invalidate (&x, sizeof (x))
#else
...
static inline virt_addr_t virtqueue_phys_to_virt (struct virtqueue *vq,
                                                  phys_addr_t       phys)
{
    return mm_kernel_p2v (phys);
}

/* Default implementation of V2P based on libmetal */
static inline phys_addr_t virtqueue_virt_to_phys (struct virtqueue *vq,
                                                  virt_addr_t       buf)
{
    return mm_kernel_v2p (buf);
}
...
void *virtqueue_get_buffer_addr (struct virtqueue *vq, uint16_t idx)
{
    ...
    return (void *)mm_kernel_p2v (vq->vq_ring.desc[idx].addr);
}
...
void virtqueue_free (struct virtqueue *vq)
{
    if (vq)
    {
        ...
        {
            KLOG_W ("%s: freeing non-empty virtqueue\r\n", vq->vq_name);
        }
                                      ...
                                      uint32_t *len)
{
    ...
    *avail_idx = vq->vq_ring.avail->ring[head_idx];
    ...
    buffer = (void *)mm_kernel_p2v (vq->vq_ring.desc[*avail_idx].addr);
...
void virtqueue_dump (struct virtqueue *vq)
{
    ...
    KLOG_D ("VQ: %s - size=%d; free=%d; queued=%d; desc_head_idx=%d; "
            "available_idx=%d; avail.idx=%d; used_cons_idx=%d; "
            "used.idx=%d; avail.flags=0x%x; used.flags=0x%x\r\n",
            vq->vq_name, vq->vq_nentries, vq->vq_free_cnt, vq->vq_queued_cnt,
            vq->vq_desc_head_idx, vq->vq_available_idx, vq->vq_ring.avail->idx,
                                    ...
                                    int readable, int writable)
{
    ...
    for (i = 0, idx = head_idx; i < needed; i++, idx = dp->next)
    {
         ...
         * driver */
        dp        = &desc[idx];
        dp->addr  = mm_kernel_v2p ((virt_addr_t)buf_list[i].buf);
