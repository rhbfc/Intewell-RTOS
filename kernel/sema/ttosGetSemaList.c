 ...
 * @brief:
 *    获取系统中信号量ID列表。
 * @param[out] idList:   存放获取到的信号量ID列表
 * @param[in]  maxIdNum: 获取到的信号量ID数量
 * @return:
 *    TTOS_INVALID_ADDRESS: idList为NULL
 *    TTOS_OK: 成功获取系统中信号量的ID列表
 * @implements: RTE_DSEMA.8.1
 */
T_TTOS_ReturnCode TTOS_GetSemaIdList (SEMA_ID idList[], T_UWORD maxIdNum)
{
    /* 输出ID地址不能为空，否则返回无效地址状态值 */
