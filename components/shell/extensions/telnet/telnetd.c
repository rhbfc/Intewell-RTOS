 ...
 * @brief telnet 协议命令
 */
static char telnetCmd[]
    = { 0xff, 0xfb, 0x01, 0xff, 0xfb, 0x03, 0xff, 0xfc, 0x1f };
...
void telnetdStop ()
{
    ...
    {
        close (
            (int)shellCompanionGet (telnetdShell, SHELL_COMPANION_ID_TELNETD));
    }
}
...
static void telnetdServer (void)
{
    ...
    if (bind (telnetdSocket, (struct sockaddr *)&telnetdAddr,
              sizeof (telnetdAddr))
            == 0
        && listen (telnetdSocket, 5) == 0)
    {
        while (1)
        {
            client = accept (telnetdSocket, (struct sockaddr *)&clientAddr,
                             (socklen_t *)&addrLen);
            if (client > 0)
            {
...
static void telnetdConnection (int client)
{
    ...
    shellCompanionAdd (telnetdShell, SHELL_COMPANION_ID_TELNETD,
                       (void *)client);
