 ...
 * @param[out] ts 指向 timespec 结构体的指针，用于返回时钟分辨率。
 ...
 */
DEFINE_SYSCALL(clock_getres, (clockid_t clk, struct timespec __user *ts))
{
    ...
    kts.tv_sec = tp.tv_sec;
    kts.tv_nsec = tp.tv_nsec;

    return copy_to_user(ts, &kts, sizeof(kts));
}
