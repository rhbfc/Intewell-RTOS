...
#define SHELL_PASSTROUGH_FUNC(_name, _prompt, _handler)                        \
    ...
    {                                                                          \
        shellPassthrough (shellGetCurrent (), #_prompt, _handler, p1,          \
                          (void *)p2);                                         \
    }
...
#define SHELL_EXPORT_PASSTROUGH(_attr, _name, _prompt, _handler, _desc)        \
    ...
    SHELL_EXPORT_CMD (_attr | SHELL_CMD_TYPE (SHELL_TYPE_CMD_MAIN), _name,     \
                      SHELL_PASSTROUGH_FUNC_NAME (_name), _desc)

unsigned int shellPassthrough (Shell *shell, const char *prompt,
                               ShellPassthrough handler, int argc,
                               char *argv[]);
