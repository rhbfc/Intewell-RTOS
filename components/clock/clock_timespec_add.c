...
/**
 * @brief 将两个timespec结构表示的时间相加
 *
 * 该函数将两个timespec结构表示的时间相加
 *
 * @param[in] time1 指向第一个timespec结构体的指针
 * @param[in] time2 指向第二个timespec结构体的指针
 * @return time1和time2累加的和
 * @retval time1和time2累加的和
 */
struct timespec clock_timespec_add (const struct timespec *time1,
