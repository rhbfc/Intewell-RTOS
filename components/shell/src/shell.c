...
SHELL_USED const ShellCommand shellUserDefault SHELL_SECTION(".shell.command") = {
...
static const char *shellText[] = {
    ...
    [SHELL_TEXT_INFO] = "\n"
                        ...
                        "        \\_\\ \\__/\\ \\/\\ \\ \\ \\_/\\  __\\ \\ \\_/ \\_/ /\\  "
                        "__/\\_\\ \\_\\_\\ \\_   \\ \\ \\`\\ \\/\\  __\\/>  </\\ \\ \\_ \n"
                        "        /\\_____\\ \\_\\ \\_\\ \\__\\ \\____\\ \\___x___/\\ "
                        "\\____/\\____/\\____\\   \\ \\_\\ \\_\\ \\____/\\_/\\_\\\\ \\__\\\n"
...
ShellCommand *shellSeekCommand(Shell *shell, const char *cmd, ShellCommand *base,
...
void shellInit(Shell *shell, char *buffer, unsigned short size)
{
    ...
    shell->commandList.count =
        ((unsigned int)(&shellCommand$$Limit) - (unsigned int)(&shellCommand$$Base)) /
        sizeof(ShellCommand);

#elif defined(__ICCARM__) || defined(__ICCRX__)
    shell->commandList.base = (ShellCommand *)(__section_begin(".shell.command"));
    shell->commandList.count = ((unsigned int)(__section_end(".shell.command")) -
                                (unsigned int)(__section_begin(".shell.command"))) /
                               sizeof(ShellCommand);
#elif defined(__GNUC__)
    shell->commandList.base = (ShellCommand *)(&_shell_command_start);
    shell->commandList.count =
        ((uintptr_t)(&_shell_command_end) - (uintptr_t)(&_shell_command_start)) /
        sizeof(ShellCommand);
#else
...
signed char shellCheckPermission(Shell *shell, ShellCommand *command)
{
    return ((!command->attr.attrs.permission || command->attr.attrs.type == SHELL_TYPE_USER ||
             (command->attr.attrs.permission & shell->info.user->attr.attrs.permission)) &&
...
void shellListKey(Shell *shell)
{
    ...
    for (short i = 0; i < shell->commandList.count; i++)
    {
        if (base[i].attr.attrs.type > SHELL_TYPE_USER &&
            base[i].attr.attrs.type <= SHELL_TYPE_KEY && shellCheckPermission(shell, &base[i]) == 0)
        {
            shellListItem(shell, &base[i]);
        }
    }
    ...
    for (unsigned short i = 0; i < shell->parser.length; i++)
    {
        if (quotes != 0 || (shell->parser.buffer[i] != ' ' && shell->parser.buffer[i] != 0))
        {
            ...
            if (record == 1)
            {
                ...
                {
                    shell->parser.param[shell->parser.paramCount++] = &(shell->parser.buffer[i]);
                }
                record = 0;
            }
                               ...
                               unsigned short compareLength)
{
    ...
    unsigned short count =
        shell->commandList.count -
        ((uintptr_t)base - (uintptr_t)shell->commandList.base) / sizeof(ShellCommand);
...
static void shellHistoryAdd(Shell *shell)
{
    ...
    if (shell->history.number > 0 &&
        strcmp(shell->history.item[(shell->history.record == 0 ? SHELL_HISTORY_MAX_NUMBER
...
static void shellHistory(Shell *shell, signed char dir)
{
    if (dir > 0)
    {
        if (shell->history.offset-- <= -((shell->history.number > shell->history.record)
                                             ? shell->history.number
                                             : shell->history.record))
        {
            shell->history.offset =
                -((shell->history.number > shell->history.record) ? shell->history.number
                                                                  : shell->history.record);
        }
    }
    else if (dir < 0)
    ...
    {
        if ((shell->parser.length = shellStringCopy(
                 shell->parser.buffer,
...
void shellExec(Shell *shell)
{
    ...
    if (shell->status.isChecked)
    {
        ...
        ShellCommand *command =
            shellSeekCommand(shell, shell->parser.param[0], shell->commandList.base, 0);
        if (command != NULL)
        {
        ...
        {
            ShellCommand *command = shellSeekCommand(shell, "exec", shell->commandList.base, 0);
            if (command != NULL)
            {
                shellRunCommand(shell, command);
            }
...
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED, 0x1B5B4300, shellRight,
                 right);
...
void shellTab(Shell *shell)
{
    ...
    else if (shell->parser.length > 0)
    {
        ...
        if (matchNum != 0)
        {
            shell->parser.length = shellStringCopy(
                shell->parser.buffer, (char *)shellGetCommandName(&base[lastMatchIndex]));
        }
        if (matchNum > 1)
        {
            shellListItem(shell, &base[lastMatchIndex]);
...
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED, 0x08000000, shellBackspace,
                 backspace);
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED, 0x7F000000, shellBackspace,
                 backspace);
...
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED, 0x1B5B337E, shellDelete,
                 delete);
...
#if SHELL_ENTER_LF == 1
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED, 0x0A000000, shellEnter,
                 enter);
#endif
#if SHELL_ENTER_CR == 1
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED, 0x0D000000, shellEnter,
                 enter);
#endif
#if SHELL_ENTER_CRLF == 1
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED, 0x0D0A0000, shellEnter,
                 enter);
#endif
...
void shellHandler(Shell *shell, char data)
{
    ...
    if (shell->info.user->data.user.password && strlen(shell->info.user->data.user.password) != 0 &&
        SHELL_GET_TICK())
    {
