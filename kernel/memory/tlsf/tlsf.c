...
_Noreturn void __tlsf_assert_fail(const char *msg, const char *file, int line, const char *func)
{
    backtrace(msg);
    printf("Assertion failed: %s (%s: %s: %d)", msg, file, func, line);
    while (1)
        ;
}
...
#define tlsf_assert(x) ((void)((x) || (__tlsf_assert_fail(#x, __FILE__, __LINE__, __func__), 0)))
...
pool_t tlsf_add_pool(tlsf_t tlsf, void *mem, size_t bytes)
{
    ...
    {
        KLOG_E("tlsf_add_pool: Memory must be aligned by %u bytes.\n", (unsigned int)ALIGN_SIZE);
        return 0;
    }
    ...
    {
#if defined(TLSF_64BIT)
        KLOG_E("tlsf_add_pool: Memory size must be between 0x%x and 0x%x00 "
               "bytes.\n",
               (unsigned int)(pool_overhead + block_size_min),
               (unsigned int)((pool_overhead + block_size_max) / 256));
#else
        KLOG_E("tlsf_add_pool: Memory size must be between %u and %u bytes.\n",
               (unsigned int)(pool_overhead + block_size_min),
...
int test_ffs_fls()
{
    ...
    if (rv)
    {
        printk("test_ffs_fls: %x ffs/fls tests failed.\n", rv);
    }
    return rv;
}
#endif
...
tlsf_t tlsf_create(void *mem)
{
    ...
    {
        KLOG_E("tlsf_create: Memory must be aligned to %u bytes.\n", (unsigned int)ALIGN_SIZE);
        return 0;
    }
...
void *tlsf_malloc(tlsf_t tlsf, size_t size)
{
    ...
    ptr = block_prepare_used(control, block, adjust);
    trace_malloc(adjust, size, 0, __builtin_return_address(0), ptr);
    return ptr;
}
...
void tlsf_free(tlsf_t tlsf, void *ptr)
{
    ...
    if (ptr)
    {
        ...
        block_insert(control, block);
        trace_free(__builtin_return_address(0), ptr);
    }
}
