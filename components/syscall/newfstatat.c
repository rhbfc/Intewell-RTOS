...
/**
 * @brief 系统调用实现：获取文件状态信息。
 *
 * 该函数实现了一个系统调用，用于获取文件状态信息。
 *
 * @param dfd 目录文件描述符。
 * @param filename 文件名。
 * @param statbuf 指向用户空间的文件状态信息结构体指针。
 * @param flag 标志。
 * @return 成功时返回 0，失败时返回负值错误码。
 */
DEFINE_SYSCALL (newfstatat, (int dfd, const char __user *filename,
                             struct stat __user *statbuf, int flag))
{
    ...
    ret = vfs_stat (fullpath, &stbuf, (AT_SYMLINK_NOFOLLOW & flag) ? 0 : 1);
