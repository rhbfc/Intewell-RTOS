                              ...
                              off_t *ppos)
{
    ...
    if (*ppos == 0)
    {
        ...
        proc_seq_printf(&data->seq, "%.2f %.2f\n",
                        (float)ts.tv_sec + (float)ts.tv_nsec / (float)NSEC_PER_SEC,
                        (float)ts.tv_sec + (float)ts.tv_nsec / (float)NSEC_PER_SEC);
    }
...
static int procfs_process_uptime_init(void)
{
    proc_create("uptime", 0444, NULL, &ops);
    return 0;
}
INIT_EXPORT_SERVE_FS(procfs_process_uptime_init, "register /proc/uptime");
