...
{
    printf ("\033[H");
    if (0 == boards[nrow][ncol]
        || 3 == boards[nrow][ncol]) //如果小老鼠前面是路或者终点，说明可以移动
    {
    ...
    else if (2 == boards[nrow][ncol]
             || 5 == boards[nrow][ncol]) //如果小老鼠前面是箱子
    {
        if (0 == boards[nnrow][nncol]
            || 3 == boards[nnrow][nncol]) //如果箱子前面是路或者终点则可以移动
        {
...
int main_pushbox (int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        switch (
            c) //每次都需要判断小老鼠前面和前面的前面的坐标的状态，这样可以在move()
        {      //函数中统一各个方向的写法
        case 'w':
