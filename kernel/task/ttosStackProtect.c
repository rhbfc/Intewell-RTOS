...
T_TTOS_ReturnCode ttosStackProtect (TASK_ID taskID)
{
    ...
    /* @REPLACE_BRACKET: 任务栈底与任务栈起始地址不相同 */
    if (pTask->stackBottom != pTask->stackStart)
    {
        KLOG_E ("%s: task already protect stack", __func__);
    ...
    /* @KEEP_COMMENT: 任务栈底加1个页的大小用于进行栈保护 */
    pTask->stackBottom += PAGE_SIZE;

    /* @KEEP_COMMENT: 任务栈起始地址的一个页为不可访问 */
    ret = ttosSetPageAttribute ((virt_addr_t)pTask->stackStart, PAGE_SIZE,
                                MT_NACCESS | MT_KERNEL);
