...
/**
 * @brief 将两个timespec64结构表示的时间相减
 *
 * 该函数将两个timespec64结构表示的时间相减
 *
 ...
 * @retval ts1和ts2相减的差
 */
struct timespec64 clock_timespec_subtract64 (const struct timespec64 *ts1,
                                             const struct timespec64 *ts2)
{
    time_t            sec;
    long              nsec;
