 ...
 * to work even with unaligned pointers on platforms (such as ARMv5) that
 * don't like unaligned loads and stores.
 */
 ...
 *
 * fdt_find_max_phandle() finds the highest phandle value in the given
 * device tree. The value returned in @phandle is only valid if the function
 * returns success.
 *
 * returns:
 ...
 *
 * Note that this code only works on device tree versions >= 16.
 *fdt_getprop() works on all versions.
 *
 * returns:
...
/**
 * fdt_node_offset_by_compatible - find nodes with a given 'compatible'
 *value
 * @fdt: pointer to the device tree blob
 * @startoffset: only find nodes after this offset
 ...
 *
 * If non-NULL, the length of the string (on success) or a negative
 * error-code (on failure) will be stored in the integer pointer to by lenp.
 *
 * Return:
 *   A pointer to the string at the given index in the string list or NULL
 * on failure. On success the length of the string will be stored in the
 * memory location pointed to by the lenp parameter, if non-NULL. On failure
 * one of the following negative error codes will be returned in the lenp
 * parameter (if non-NULL): -FDT_ERR_BADVALUE if the property value is not
 * NUL-terminated -FDT_ERR_NOTFOUND if the property does not exist
 */
...
/**
 * fdt_address_cells - retrieve address size for a bus represented in the
 *tree
 * @fdt: pointer to the device tree blob
