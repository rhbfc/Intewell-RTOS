...
int local_fifo_read(struct file *filep, uint8_t *buf,
                    size_t *len, bool once)
{
  ...
  while (remaining > 0)
  {
    ...
    if (nread < 0)
    {
      ...
      {
        KLOG_W("ERROR: file_read() failed: %d", ret);
        goto errout;
      }
    }
    else if (nread == 0)
...
int local_sync(struct file *filep)
{
  ...
  ret     = local_fifo_read(filep, (uint8_t *)&pktlen, &readlen, false);
...
int local_getaddr(struct local_sock_s *lsock, struct sockaddr *addr,
                  socklen_t *addrlen)
{
  ...
  pathlen = strnlen(lsock->path, UNIX_PATH_MAX - 1);
  ...
  memcpy(unaddr->sun_path, lsock->path, pathlen);
