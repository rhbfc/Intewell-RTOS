...
/****************************************************************************
 * 本文件基于 Apache NuttX 对应 mtd 接口定义移植到当前 RTOS，
 * 并已针对本地目录结构和平台环境进行了修改。
 ****************************************************************************/
...
#define MTDIOC_GEOMETRY                                                        \
    _MTDIOC (0x0001) /* IN:  Pointer to write-able struct                      \
...
#define MTDIOC_BULKERASE                                                       \
    _MTDIOC (0x0003) /* IN:  None                                              \
                      * OUT: None */
#define MTDIOC_PROTECT                                                         \
    _MTDIOC (0x0004) /* IN:  Pointer to read-able struct                       \
                      *      mtd_protects_s that provides                      \
                      *      the region to protect.                            \
                      * OUT: None */
#define MTDIOC_UNPROTECT                                                       \
    _MTDIOC (0x0005) /* IN:  Pointer to read-able struct                       \
                      *      mtd_protects_s that provides                      \
                      *      the region to un-protect.                         \
                      * OUT: None */
#define MTDIOC_SETSPEED                                                        \
    _MTDIOC (0x0006) /* IN:  New bus speed in Hz                               \
                      * OUT: None */
#define MTDIOC_EXTENDED                                                        \
    _MTDIOC (0x0007) /* IN:  unsigned long                                     \
                      *      0=Use normal memory region                        \
                      *      1=Use alternate/extended memory                   \
                      * OUT: None */
#define MTDIOC_ECCSTATUS                                                       \
    _MTDIOC (0x0008) /* IN:  Pointer to uint8_t                                \
                      * OUT: ECC status */
#define MTDIOC_ERASESTATE                                                      \
    _MTDIOC (0x000a) /* IN:  Pointer to uint8_t                                \
                      * OUT: Byte value that represents the                    \
                      *      erased state of the MTD cell */
#define MTDIOC_ERASESECTORS                                                    \
    _MTDIOC (0x000c) /* IN: Pointer to mtd_erase_s structure                   \
                      * OUT: None */
...
#define MTD_BWRITE(d, s, n, b)                                                 \
    ((d)->bwrite ? (d)->bwrite (d, s, n, b) : (-ENOSYS))
...
struct mtd_dev_s {
    ...
    ssize_t (*bread) (struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
                      uint8_t *buffer);
    ssize_t (*bwrite) (struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
    ...
    int (*isbad) (struct mtd_dev_s *dev, off_t block);
    int (*markbad) (struct mtd_dev_s *dev, off_t block);
...
struct mtd_dev_s *mtd_partition (struct mtd_dev_s *mtd, off_t firstblock,
                                 off_t nblocks);
...
struct mtd_dev_s *mtd_rwb_initialize (struct mtd_dev_s *mtd);
#endif
 ...
 *
 * Description:
 *   Initialize to provide a Sector Mapped Allocation for Really Tiny
 *(SMART) Flash block driver wrapper around an MTD interface
 *
 * Input Parameters:
 *   minor - The minor device number.  The MTD block device will be
...
int smart_initialize (int minor, struct mtd_dev_s *mtd, const char *partname);
...
struct mtd_dev_s *s512_initialize (struct mtd_dev_s *mtd);
#endif
...
struct mtd_dev_s *at45db_initialize (struct spi_dev_s *dev);
...
struct mtd_dev_s *at24c_initialize (struct i2c_master_s *dev, uint8_t address);
#else
...
struct mtd_dev_s *at25ee_initialize (struct spi_dev_s *dev, int devtype,
                                     int readonly);
...
#ifdef CONFIG_AT24XX_MULTI
void at24c_uninitialize (struct mtd_dev_s *mtd);
#endif
...
struct mtd_dev_s *at25_initialize (struct spi_dev_s *dev);
...
struct mtd_dev_s *is25xp_initialize (struct spi_dev_s *dev, uint16_t spi_devid);
...
struct mtd_dev_s *m25p_initialize (struct spi_dev_s *dev);
...
struct mtd_dev_s *mx35_initialize (struct spi_dev_s *dev);
...
struct mtd_dev_s *rammtd_initialize (uint8_t *start, size_t size);
...
struct mtd_dev_s *sst25_initialize (struct spi_dev_s *dev);
...
struct mtd_dev_s *sst25xx_initialize (struct spi_dev_s *dev);
...
struct mtd_dev_s *sst26_initialize_spi (struct spi_dev_s *dev,
                                        uint16_t          spi_devid);
...
struct mtd_dev_s *sst39vf_initialize (void);
...
struct mtd_dev_s *gd25_initialize (struct spi_dev_s *dev, uint32_t spi_devid);
...
struct mtd_dev_s *gd5f_initialize (struct spi_dev_s *dev, uint32_t spi_devid);
...
struct mtd_dev_s *s25fl1_initialize (struct qspi_dev_s *qspi, bool unprotect);
...
struct mtd_dev_s *mx25l_initialize_spi (struct spi_dev_s *dev);
...
struct mtd_dev_s *mx25rxx_initialize (struct qspi_dev_s *qspi, bool unprotect);
...
struct mtd_dev_s *n25qxxx_initialize (struct qspi_dev_s *qspi, bool unprotect);
...
struct mtd_dev_s *w25qxxxjv_initialize (struct qspi_dev_s *qspi,
                                        bool               unprotect);
...
struct mtd_dev_s *filemtd_initialize (const char *path, size_t offset,
...
void filemtd_teardown (struct mtd_dev_s *dev);
...
bool filemtd_isfilemtd (struct mtd_dev_s *mtd);
...
struct mtd_dev_s *nullmtd_initialize (size_t mtdlen, int16_t sectsize,
                                      int32_t erasesize);
...
int rpmsgmtd_register (const char *remotecpu, const char *remotepath,
...
int dhara_initialize (int minor, struct mtd_dev_s *mtd);
#endif
